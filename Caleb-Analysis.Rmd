...
```{r Kahle, include = FALSE}
data("iris")
head(iris)
iris.scaled <- scale(iris[, -5])
km.res <- kmeans(iris.scaled, 3, nstart = 10)
p1 <- factoextra::fviz_cluster(km.res, iris[, -5], ellipse.type = "norm")
library("tidyverse")
library("patchwork")
library("scales")
library("glue")
...
library("ggdensity")
p2 <- pca_df |> 
  mutate(species = factor(km.res$cluster)) |> 
  ggplot(aes(comp_1, comp_2, fill = species, color = species)) +
    geom_hdr(method = method_mvnorm(), alpha = .25, probs = .90) +
    geom_hdr_lines(method = method_mvnorm(), probs = .90) +
    geom_point()
...
p2 <- pca_df |> 
  mutate(species = factor(km.res$cluster)) |> 
  ggplot(aes(comp_1, -comp_2, fill = species, color = species)) +
  geom_hdr(method = method_mvnorm(), alpha = .25, probs = .95) +
  geom_hdr_lines(method = method_mvnorm(), probs = .95) +
  geom_point()
...
nba <- nba %>% mutate(across(where(is.numeric), 
                                    perMin, .names = "{col}PerMin"))
...
# kable(slice_usage, col.names = colnames(slice_usage), escape = F) %>%
...
# kable(slice_role, col.names = colnames(slice_role), escape = F) %>%
#   kable_material(c("striped", "hover")) %>%
#   kable_styling(latex_options = "HOLD_position") %>%
#   row_spec(0, bold = T)
```
...
# kable(dallas_t, col.names = colnames(dallas_t), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position") %>%
#   row_spec(0, bold = T)
```
...
```{r}
# For interest, I'm guessing they chose these players.
Stu_players <- c("Sterling Brown", "Maxi Kleber", "Dwight Powell", "Josh Green")
Stu_picks0 <- dallas_t %>% filter(Name %in% Stu_players)

Stu_picks0 %>% flextable() %>%
  align(align = "center", part = "all") %>%
  width(j = 1, width = 1.3) %>%
  width(j = c(2:3), width = .6) %>%
  width(j = c(4:15), width = .5)

# kable(Stu_picks0, col.names = colnames(Stu_picks0), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position") %>%
#   row_spec(0, bold = T)
```
...
How "good" is the similarity metric?
Explore the data for the players that are "closest/most similar" - what in the data makes them close? Do the same for the dissimilar players.
...
# centers <- tibble(
#   Name = c("center1", "center2"),
#   `3PAPerMin` = dallas2Means$centers[1:2],
#   ORPerMin = c(dallas2Means$centers[1,][2], dallas2Means$centers[2,][2]),
#   cluster = c(1, 2),
#   center = c("yes", "yes"))

# chicken <- dallasKMeans_prep %>%
#   mutate(cluster = dallas2Means$cluster,
#          center = "") %>%
#   rownames_to_column(var = "Name") %>%
#   bind_rows(centers)
# 
# plot_ly(data = chicken,
#         x = ~`3PAPerMin`,
#         y = ~ORPerMin,
...
#         textposition = "auto",
#         hoverinfo = "text",
#         hovertext = paste(chicken$Name))
# 
# ggplot(chicken, aes(`3PAPerMin`, ORPerMin, color = factor(cluster),
#                     fill = factor(cluster))) +
#   geom_hdr(method = method_mvnorm(), probs = .95) +
#   geom_point(aes(shape = factor(center))) +
#   geom_label(aes(label = Name)) +
#   labs(color = "cluster", shape = "center")
# 
...
plot(dallasClust)
#kable(dallasClust) %>% kable_material(c("striped", "hover"))
```
...
```{r}
# creating factoring levels of the dataset. This will help us keep the columns sorted in the way we want.
...
# kable(usageKMeans_prep %>% slice(1:5), col.names = colnames(usageKMeans_prep), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
...
plot(usageClust)
# kable(usageClust) %>% kable_material(c("striped", "hover"))
```

The tests favor three clusters. Some tests also prefer two and four clusters, so those models are worth a look.

```{r}
# creating the clusters. Setting seeds so the cluster #s are most readable.
set.seed(121)
...
```{r}
# reproducing K = 2 means without insignificant variables
set.seed(121)
usage2Means <- usageKMeans_prep %>%
  select(-`SH-EFF`) %>%
  kmeans(centers = 2, nstart = 50)
# creating a second usage without those statistics so i don't have to reproduce it 800 million times.
...
# kable(usage2centers) %>% kable_material(c("striped", "hover")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(dist_slice1, col.names = colnames(dist_slice1), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(prototype_k2c1, col.names = colnames(prototype_k2c1), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(dist_slice2, col.names = colnames(dist_slice2), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(prototype_k2c2, col.names = colnames(prototype_k2c2), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(dist_slice3, col.names = colnames(dist_slice3), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(outlier_k2c1, col.names = colnames(outlier_k2c1), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(dist_slice4, col.names = colnames(dist_slice4), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(outlier_k2c2, col.names = colnames(outlier_k2c2), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(stu_table) %>% kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(usage3centers) %>% kable_material(c("striped", "hover")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(master_distances, col.names = colnames(master_distances), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(master_distances1, col.names = colnames(master_distances1), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(compare_K2K3) %>% kable_material(c("striped", "hover")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
roleKMeans_prep %>%
  ...
  # kable(col.names = colnames(roleKMeans_prep), escape = F) %>%
  # kable_material(c("hover", "striped")) %>%
  # kable_styling(latex_options = "HOLD_position")
...
# prints number of clusters that Elbow method suggests. But its annoying and pops up another tab so I commented it out mwahahaha
# roleClust %>% filter(Method == "Elbow") %>% select(Method, n_Clusters) %>% view()
```

```{r}
plot(roleClust) +
  labs(title = "Optimal Number of Clusters", x = "")
# kable(roleClust) %>% kable_material(c("striped", "hover"))
```
...
# kable(stu_role_table) %>% kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# recreating role without those statistics to make things simpler. They need to go back to the beginning of this code to restore it if they do remove a variable.
...
# kable(roleKcenters) %>% kable_material(c("striped", "hover"))
```
...
# kable(mast_dist_slice, col.names = colnames(mast_dist_slice), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(mast_dist_slice1, col.names = colnames(mast_dist_slice1), escape = F) %>%
#   kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(compare_table) %>% kable_material(c("striped", "hover")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(Caleb_table) %>% kable_material(c("hover", "striped")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(dallas_role2022) %>% kable_material(c("striped", "hover")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(dallas_role2023) %>% kable_material(c("striped", "hover")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
# kable(looking_clus) %>% kable_material(c("striped", "hover")) %>%
#   kable_styling(latex_options = "HOLD_position")
```
...
<!-- *Assess offseason trade?* -->

<!-- ## Assess an Offseason Trade -->

<!-- It can be difficult to assess trades in the NBA using clustering, because so many trades involve draft picks of future players that obviously are not in our data set. However, on July 6, 2022 the Denver Nuggets and Washington Wizards made a crucial trade that involved no draft picks. Let's analyze that here. -->

<!-- ```{r} -->
<!-- nuggets2022 <- role7Means_players %>% -->
<!--   filter(Team == "den") -->
<!-- wizards2022 <- role7Means_players %>% -->
<!--   filter(Team == "wsh") -->
<!-- ``` -->

<!-- *This looks kinda boring because 3/4 players lie in cluster 3* -->